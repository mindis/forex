...
### data preprocessing for predicting cp_wma
def preprocess(df):
    x = df[["cp_wma"]]
...
### model 1 mean squared error is 5e-07,
###         mean  absolute error is 0.001
###         accuracy is 74%
### file name is reg4e07.hdf5
def model_1():
    ...
    #pool = tf.keras.layers.MaxPool1D()(res_block1)

    res_block = mlti_res_block(res_block,17,35,53,105)
    #pool = tf.keras.layers.MaxPool1D()(res_block2)

    res_block = mlti_res_block(res_block,31,72,106,209)
    #pool = tf.keras.layers.MaxPool1D()(res_block3)
...
#    def midium(inputs):
#        cnn = Conv1D(18,3,padding="causal",activation="relu")(inputs)
#        pool = layers.MaxPool1D(4)(cnn)
#        cnn = Conv1D(36,3,padding="causal",activation="relu")(inputs)
#        pool = layers.MaxPool1D(4)(cnn)
#        
#    def coarse(inputs):
#        cnn = Conv1D(36,3,padding="causal",activation="relu")(inputs)
#        pool = layers.MaxPool1D(16)(cnn)        
#
#    fine_net = fine(inputs=inputs)
#    midium_net = midium(inputs=inputs)
#    coarse_net = coarse(inputs=inputs)
#    
#    fine_net = layers.Flatten()(fine_net)
#    midium_net = layers.Flatten()(midium_net)
#    coarse_net = layers.Flatten()(coarse_net)
#
#    concat = layers.Concatenate()([fine_net,midium_net,coarse_net])
...
# add = layers.Concatenate()([gru1,gru2])
...
add = layers.Add()([cnn1,cnn2,cnn3])
# add = layers.PReLU()(add)
add = layers.Concatenate()([add,cnn4])

# cnn1 = layers.Conv1D(36,8,padding="same",activation="relu")(add)

# cnn2 = layers.Conv1D(36,4,padding="same",activation="relu")(add)

# cnn3 = layers.Conv1D(36,2,padding="same",activation="relu")(add)

# cnn4 = layers.Conv1D(108,1,padding="same",activation="relu")(add)
